...
export default function WhoPage(){
  ...
  useEffect(()=>{
    async function run(){
      try{
        // Check for results in localStorage (no ID required)
        const url = new URL(window.location.href);
        const idFromUrl = url.searchParams.get('id');
        
        // If there's an ID in URL, store it, but don't require it
        if (idFromUrl) {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ maxWidth: '800px', width: '100%' }}>
        ...
        <h1 style={{ margin: '20px 0', fontSize: '32px', fontWeight: 'normal', textAlign: 'center' }}>Who You Are</h1>
        <p style={{ margin: '0 0 20px 0', fontSize: '16px', color: '#aaa', textAlign: 'center' }}>Short read. Clear pattern. Next step included.</p>

        {error ? <p style={{color:'#ff7675', textAlign: 'center'}}>{error}</p> : null}
        {!who ? <p style={{color:'#aaa', textAlign: 'center'}}>Building your profile…</p> : (
          <>
          <IdentityMirror 
        ...
        <div style={{marginTop:24, display:'flex', justifyContent:'center'}}>
          <a href="/results" className="btn">View Detailed Results →</a>
        </div>
      </div>
    </div>
  );
