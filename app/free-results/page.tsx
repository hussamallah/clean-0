...
function IdentityBlueprintContent() {
  ...
  function renderRedesignedIdCard() {
    ...
    return (
      <div id="id-card" className="rounded-lg border border-white/10 bg-white/5 p-2 sm:p-3" style={{ ...neonBorderStyle(), maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
        <div className="mb-2">
          <div className="text-sm text-ink-muted" style={{letterSpacing:.5}}>🎯 YOUR POINT ZERO PROFILE NAVIGATOR ⚖️</div>
  ...
  if (!isReady) return (
    <div className="min-h-screen bg-canvas text-ink flex items-center justify-center">
      <div className="text-center">
        ...
        <div className="text-sm text-ink-muted">Fetching example profile data</div>
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-canvas text-ink flex flex-col items-start sm:items-center p-2 sm:p-4">
      {/* Page Header */}
      <div className="w-full max-w-6xl mx-auto mb-6 text-center">
        <div className="flex items-center justify-center gap-3 mb-2">
          ...
          <span className="inline-flex items-center rounded-full border border-line bg-surface px-2.5 py-0.5 text-xs font-medium text-brand-deep">
            Sample
          </span>
        </div>
        <p className="text-ink-muted text-sm">
          This is what you get for free when you complete the Point Zero assessment. See your ID card, domain summaries, and full detailed results below.
        </p>
      </div>

      {/* Tabs */}
      <div className="w-full max-w-6xl mx-auto mb-6">
        <div className="flex justify-center gap-2 border-b border-line">
          <button
      ...
      {activeTab === 'id' && (
        <>
          ...
          <Tooltip text="This is an example ID card showing what your Point Zero profile looks like.">
            <div className="text-xs text-ink-soft font-mono mb-4 cursor-pointer">
              Example Run ID: {rid}
            </div>
          </Tooltip>
        </>
