...
export default function IdentityMirror({ lines, onSeen }: { lines: string[]; onSeen?: () => void }) {
  // Use exactly 4 sentences as per HTML spec
  const defaultLines = [
  ...
  const four = (lines && lines.length >= 4) ? lines.slice(0, 4) : defaultLines;
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: '10px 0', lineHeight: 1.6, color: '#b6c2d1', fontSize: '14px' }}>
        Below are scores that show you how you act, process, plan, connect, and push through challenges daily.
      </p>
    </div>
  );
}
