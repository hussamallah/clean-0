...
export default function FullAssessment(){
  ...
  useEffect(()=>{
    ...
    async function finalize(){
      ...
      try {
        ...
        const resultsJson = JSON.stringify(normalized);
        const hash = await sha256(stableStringify(normalized));
        
        try {
          localStorage.setItem('gz_full_results', resultsJson);
          localStorage.setItem('gz_full_hash', hash);
          
          // Also store in runs map for hash lookup
  ...
  }, [done, results, domainOrder.length]);
          ...
          {!nudge ? (
            <DomainRunner domain={domainOrder[idx]} onComplete={(payload)=>{
              ...
              if (typeof mean === 'number') {
                setNudge({ domain: domainOrder[idx], mean });
              }
              setPending(payload);
            }} />
          ) : null}
